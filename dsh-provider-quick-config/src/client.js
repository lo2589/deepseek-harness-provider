  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      function ScreenshotButton(props) {
        ...
        // 选区截图：保留 stream 焦点 + 烤帧静态显示
        //   1. 保持 stream 活跃（不立即 stop track）—— 让 macOS 焦点留在 DSH 标签页
        //   2. video 拿到首帧后立即烤帧到 frameCanvas + video 暂停
        //   3. frameCanvas 覆盖 video 显示（画面静止，拖框时不会有"上一帧"进来）
        //   4. 用户拖框选区 → 从 frameCanvas 截
        //   5. 拖框/取消后 stop track（stream 关闭）
        // 关键：如果立即 stop track（c809313 之前那样），macOS 焦点会跳走，
        //       stream 内容会被其他应用覆盖（截到剪映）。
        function selectRegion(stream) {
          return new Promise(function (resolve) {
            ...
            function stopStream() {
              ...
              try { if (track !== null) track.stop() } catch (e) {}
            }
            function cleanup() {
            ...
            function doCrop() {
              ...
              // 从烤帧 canvas 截（静态，不依赖 stream）
              out.getContext('2d').drawImage(frameCanvas, sx, sy, sw, sh, 0, 0, out.width, out.height)
            ...
            // 烤帧：拿到首帧 → 画到 frameCanvas → 覆盖 video → 暂停 video
            // track 不停 —— 保持 DSH 焦点（避免截到剪映等非 DSH 内容）
            function bakeFrame() {
      ...
      function ShowcasePanel() {
        ...
        // 按来源分组：history 按轮次分；screenshot / upload 单独成框
        var historyGroups = []
        ...
        for (var i = 0; i < items.length; i++) {
          var it = items[i]
          if (it.source === 'screenshot') { screenshotItems.push(it); continue }
          if (it.source === 'upload') { uploadItems.push(it); continue }
          var t = typeof it.turn === 'number' ? it.turn : 0
          var g = historyMap.get(t)
          if (g === undefined) { g = { turn: t, items: [] }; historyMap.set(t, g); historyGroups.push(g) }
          g.items.push(it)
        }
        function groupDir(items) {
          if (items.length === 0) return null
          var p = items[0].path
          var idx = p.indexOf('/.screenshots/')
          if (idx >= 0) return p.slice(0, idx + '/.screenshots'.length)
          idx = p.indexOf('/.uploads/')
          if (idx >= 0) return p.slice(0, idx + '/.uploads'.length)
          return null
        }
        function renderBucket(title, bucketItems, openDir) {
          return h('div', { key: title, className: 'sc-bucket' },
            h('div', { className: 'sc-bucket-title' },
              title,
              openDir !== null ? h('button', { type: 'button', className: 'sc-bucket-open', title: '在文件管理器中打开文件夹', onClick: () => revealFile(openDir) }, '📂') : null),
            bucketItems.map(function (item) {
        ...
        function renderFolder(title, dirPath) {
          return h('div', { key: title, className: 'sc-bucket sc-folder' },
            h('div', { className: 'sc-bucket-title' }, title),
            h('button', { type: 'button', className: 'sc-folder-open', title: '在文件管理器中打开文件夹', onClick: () => revealFile(dirPath) },
              h('span', { role: 'img', 'aria-hidden': true }, '📂'),
              ' 打开文件夹'))
        }
        function renderHistoryGroup(g) {
          return renderBucket('第 ' + (g.turn + 1) + ' 轮', g.items, null)
        }
        ...
        return h('div', null,
          ...
          h('div', { className: 'sc-panel' },
            ...
            h('div', { className: 'sc-body' },
              ...
              screenshotItems.length > 0 ? renderFolder('截图库', groupDir(screenshotItems)) : null,
              uploadItems.length > 0 ? renderFolder('上传库', groupDir(uploadItems)) : null),
