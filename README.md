...
- **Screenshot 📷**: captures the screen via `getDisplayMedia`, **saves the PNG into `<session cwd>/.截图/`** (the media-showcase library folder, gitignored) and writes the returned path into the composer — you decide whether to send it. The saved screenshot automatically appears in the showcase.
